...
compile_digit_span_data <- function() {
	multiple.sessions <- character()
	filenames <- list.files()
    digit.span.data <- matrix(nrow=length(filenames), ncol=13)
	user <- character()
...
digit.span.data <- compile_digit_span_data()
