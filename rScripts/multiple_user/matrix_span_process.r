...
compile_matrix_span_data <- function() {

	multiple.sessions <- character()
	filenames <- list.files()
	matrix.span.data <- matrix(nrow=length(filenames), ncol=12) #ncol needs to be number of values in p.summary above
	user <- character()
	...
	for (i in 1:length(filenames)) {
		...
		tmp.matrix.summary <- matrix_span_processing(tmp.df)
...
matrix.span.data <- compile_matrix_span_data()
