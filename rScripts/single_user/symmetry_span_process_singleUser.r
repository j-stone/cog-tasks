...
compile_symm_span_data <- function(filestring) {

    raw.data <- read.csv(filestring)
    ps <- unique(raw.data$session.subject.subject.code)
    symmetry.span.data <- matrix(nrow=length(ps), ncol=14)    	
	...
	for (i in 1:length(ps)) {
		...
		tmp.symm.summary <- symm_span_processing(sub.raw.data)
...
symmetry.span.data <- compile_symm_span_data("datafile.csv")
