...
compile_reading_span_data <- function() {

	multiple.sessions <- character()
	filenames <- list.files()
	reading.span.data <- matrix(nrow=length(filenames), ncol=14) #ncol needs to be number of values in p.summary above
	user <- character()
	...
	for (i in 1:length(filenames)) {
		...
		tmp.read.summary <- reading_span_processing(tmp.df)
...
reading.span.data <- compile_reading_span_data()
