...
compile_arrow_span_data <- function(filestring) {


	raw.data <- read.csv(filestring)
	ps <- unique(raw.data$session.subject.subject.code)
	arrow.span.data <- matrix(nrow=length(ps), ncol=12) #ncol needs to be number of values in p.summary above	
	...
	for (i in 1:length(ps)) {
		...
		tmp.arrow.summary <- arrow_span_processing(sub.raw.data)
...
arrow.span.data <- compile_arrow_span_data("datafile.csv")
