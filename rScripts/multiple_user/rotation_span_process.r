...
compile_rotation_span_data <- function() {

	multiple.sessions <- character()
	filenames <- list.files()
	rotation.span.data <- matrix(nrow=length(filenames), ncol=14) #ncol needs to be number of values in p.summary above
	user <- character()
	...
	for (i in 1:length(filenames)) {
		...
		tmp.rot.summary <- rotation_span_processing(tmp.df)
...
rotation.span.data <- compile_rotation_span_data()
